...
function Search() {
  ...
  const installers = [
    { id: 1, initials: 'JM', name: 'James Mwangi', location: 'Westlands, Nairobi', distance: 2.1, rating: 4.9, reviews: 42, tags: ['Fibre', 'Home WiFi'], priceFrom: 1500, priceTo: 5000, experience: 5, jobs: 127, available: 'Available today', color: 'bg-green-100 text-green-700', responseTime: '< 1 hr' },
    { id: 2, initials: 'AO', name: 'Amina Omar', location: 'Kilimani, Nairobi', distance: 3.8, rating: 4.8, reviews: 38, tags: ['Business', 'CCTV'], priceFrom: 2000, priceTo: 8000, experience: 7, jobs: 98, available: 'Available today', color: 'bg-blue-100 text-blue-700', responseTime: '< 2 hrs' },
    { id: 3, initials: 'PK', name: 'Peter Kamau', location: 'Kasarani, Nairobi', distance: 5.2, rating: 4.6, reviews: 21, tags: ['Fibre', 'Routers'], priceFrom: 1200, priceTo: 4000, experience: 3, jobs: 54, available: 'Available today', color: 'bg-yellow-100 text-yellow-700', responseTime: '< 3 hrs' },
    { id: 4, initials: 'SM', name: 'Sarah Mutua', location: 'Kilimani, Nairobi', distance: 6.1, rating: 5.0, reviews: 55, tags: ['Home WiFi', 'Mesh Networks'], priceFrom: 1800, priceTo: 6000, experience: 6, jobs: 143, available: 'Available today', color: 'bg-purple-100 text-purple-700', responseTime: '< 1 hr' },
    { id: 5, initials: 'DK', name: 'David Kipchoge', location: 'Ruiru, Kiambu', distance: 8.3, rating: 4.5, reviews: 17, tags: ['Fibre', 'Business'], priceFrom: 1000, priceTo: 3500, experience: 2, jobs: 31, available: 'Available tomorrow', color: 'bg-red-100 text-red-700', responseTime: '< 4 hrs' },
    { id: 6, initials: 'FW', name: 'Faith Wanjiku', location: 'Thika, Kiambu', distance: 12, rating: 4.9, reviews: 29, tags: ['Home WiFi', 'Routers'], priceFrom: 1300, priceTo: 4500, experience: 4, jobs: 76, available: 'Available today', color: 'bg-orange-100 text-orange-700', responseTime: '< 2 hrs' },
  ]

  const filteredInstallers = installers
    .filter(installer => {
      const matchesFilter = activeFilter === 'All' || installer.tags.some(tag => tag.toLowerCase().includes(activeFilter.toLowerCase()))
      const matchesSearch = searchTown === '' || installer.location.toLowerCase().includes(searchTown.toLowerCase())
      return matchesFilter && matchesSearch
    })
    .sort((a, b) => {
      if (sortBy === 'Nearest first') return a.distance - b.distance
      if (sortBy === 'Highest rated') return b.rating - a.rating
      if (sortBy === 'Most reviews') return b.reviews - a.reviews
      if (sortBy === 'Lowest price') return a.priceFrom - b.priceFrom
      return 0
    })
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-green-600 px-8 py-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center bg-white rounded-xl px-5 py-4 max-w-2xl gap-3 shadow-sm">
            ...
            />
            <button
              onClick={() => setSearchTown(searchTown)}
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-8 text-sm text-gray-500">
          <span>🔍 Showing <strong className="text-gray-900">{filteredInstallers.length} installers</strong> near you</span>
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        {filteredInstallers.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="font-semibold text-gray-900 text-lg mb-2">No installers found</h3>
            <p className="text-gray-500 text-sm mb-4">Try searching a different town or changing your filter</p>
            <button onClick={() => { setSearchTown(''); setActiveFilter('All') }} className="bg-green-600 text-white px-5 py-2.5 rounded-lg hover:bg-green-700 text-sm font-medium">
              Clear filters
            </button>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-green-600 font-medium">{installer.distance} km away</div>
                    </div>
                  </div>
        ...
        {filteredInstallers.length > 0 && (
          <div className="text-center mt-10">
            ...
            <p className="text-xs text-gray-400 mt-2">Showing {filteredInstallers.length} of 1,240+ installers in Kenya</p>
          </div>
        )}
      </div>
    </div>
