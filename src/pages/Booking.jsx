      ...
      <div className="bg-green-600 px-8 py-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="bg-white rounded-xl px-6 py-4 flex items-center gap-4">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold ${installer.color}`}>{installer.initials}</div>
            <div>
              <div className="font-semibold text-gray-900">{installer.name}</div>
              <div className="text-xs text-gray-400">🗺️ {installer.location} · ★ {installer.rating} · {installer.reviews} reviews</div>
            </div>
            <div className="text-right ml-4 pl-4 border-l border-gray-100">
              <div className="text-lg font-bold text-green-600">KES {installer.priceFrom.toLocaleString()}</div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={county} onChange={(e) => setCounty(e.target.value)} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-green-500 transition">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Westlands, Kilimani, Ruiru" value={town} onChange={(e) => setTown(e.target.value)} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-green-500 transition"/>
                  </div>
                </div>
                <div>
                  ...
                  <input type="text" placeholder="e.g. Rose Garden Apartments, Apt 4B — Ngong Road" value={streetAddress} onChange={(e) => setStreetAddress(e.target.value)} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-green-500 transition"/>
                </div>
                <div>
                  ...
                  <input type="text" placeholder="e.g. Near Nakumatt Prestige, opposite KCB Bank" value={landmark} onChange={(e) => setLandmark(e.target.value)} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-green-500 transition"/>
                </div>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" value={date} onChange={(e) => setDate(e.target.value)} min={new Date().toISOString().split('T')[0]} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-green-500 transition"/>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1">Preferred time <span className="text-red-400">*</span></label>
                  <select value={time} onChange={(e) => setTime(e.target.value)} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-green-500 transition">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 sticky top-6">
              ...
              <div className="space-y-3 mb-4">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-800">{installer.name}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Services</span>
                  <span className="font-medium text-gray-800 text-right max-w-32 text-xs">{selectedServices.length > 0 ? selectedServices.join(', ') : 'None selected'}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-800 text-right max-w-32 text-xs">{town && county ? `${town}, ${county}` : 'Not set'}</span>
                </div>
                <div className="border-t border-gray-100 pt-3 flex justify-between">
                  <span className="text-gray-500 text-sm">Starting price</span>
                  <span className="font-bold text-green-600">KES {installer.priceFrom.toLocaleString()}</span>
                </div>
              </div>
              <button
