    ...
    <section className="px-8 py-16 bg-white">
      ...
      <div className="grid grid-cols-4 gap-4">
        {installers.slice(0, 3).map((installer) => (
          <div key={installer.id} className="border border-gray-200 rounded-xl p-5 hover:border-green-500 transition cursor-pointer">
            ...
            <div className="text-yellow-500 text-xs mb-2">{'★'.repeat(Math.floor(installer.rating))} {installer.rating}</div>
            <div className="flex flex-wrap gap-1 mb-3">
              {installer.tags.map((tag) => (
                <span key={tag} className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-full">{tag}</span>
              ))}
            </div>
            <span className={`text-xs px-2 py-1 rounded-full block mb-3 ${installer.available === 'Available today' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
              {installer.available}
            </span>
